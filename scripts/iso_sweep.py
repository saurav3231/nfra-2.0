    ...
    for url in _WIKI_URLS:
        try:
            ...
            with zipfile.ZipFile(tmp) as z:
                for src, dst in [
                    ("wikitext-2-raw/wiki.train.raw", _WIKI_FILES[0]),
...
CONFIGS = [
    ("baseline", {}),
    ("gland", {"iso_gland": True}),
    ("vgate", {"iso_vgate": True}),
    ("rgate", {"iso_rgate": True}),
    ("phase", {"iso_phase": True}),
    ("exit", {"iso_exit": True}),
]
...
def main():
    ...
    print(
        "nfra 5M cortex geometry: dim %d, depth %d, unique %d, %.2fM params"
        % (dim, depth, U, spec["params"] / 1e6)
    )
    print(
        "protocol: %d steps, batch 8, seq 256, fp16 AMP, EMA 0.99, "
    ...
    for name, off in CONFIGS:
        ...
        rec = train_one(
            m, vocab, STEPS, tr, ev, EVAL_GAP, ema_decay=arena.EMA_DECAY, seed=SEED
        )
        final = rec["eval_hist"][-1][1] if rec["eval_hist"] else float("nan")
        rows.append(
            (name, n_params, final, rec["tok_s"], rec["peak_mem"], rec["wall_s"])
        )
        print(
            "  [{}] final {:.3f}   {:.0f} tok/s   {:.2f} GB   {:.2f}M   ({:.0f}s)".format(
                name, final, rec["tok_s"], rec["peak_mem"], n_params, rec["wall_s"]
            )
        )
        del m
        if HAS_CUDA:
    ...
    print(
        "%-9s %7s %8s %9s %8s %8s"
        % ("config", "params", "loss", "tok/s", "mem/GB", "delta")
    )
    for name, n_params, loss, tok, mem, _w in rows:
        ...
        flag = (
            "KEEP"
            if delta >= 0.02
            else ("(washes)" if abs(delta) < 0.02 else f"keep? +{delta:.3f}")
        )
        print(
            "%-9s %6.2fM %8.3f %9.0f %8.2f %+7.3f  %s"

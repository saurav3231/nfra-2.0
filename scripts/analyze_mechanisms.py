...
def run_ablate():
    ...
    for name, build_kw, train_kw, n_seeds_req in ABLATE:
        ...
        spec = build_family_spec("nfra", size, V)
        ...
        for seed in seeds:
            ...
            model = spec["builder"](V, spec["dim"], **spec["extra"], **build_kw).to(
                DEVICE
            )
            rescale_embed(model)
...
def classify_mechanisms(results):
    ...
    baseline_evals = [
        r["final_eval"] for r in baseline if r.get("final_eval") is not None
    ]
    ...
    baseline_std = (
        sum((v - baseline_mean) ** 2 for v in baseline_evals)
        / max(len(baseline_evals) - 1, 1)
    ) ** 0.5
    ...
    for name, variant_results in results.items():
        ...
        evals = [
            r["final_eval"] for r in variant_results if r.get("final_eval") is not None
        ]
        if not evals:
            classifications[name] = {
                "status": "no_data",
        ...
        variant_std = (
            sum((v - variant_mean) ** 2 for v in evals) / max(len(evals) - 1, 1)
        ) ** 0.5
        delta_mean = variant_mean - baseline_mean
        delta_std = (
            math.sqrt(variant_std**2 + baseline_std**2)
            if baseline_std > 0
            else variant_std
        )
        ...
        if better_seeds == total_seeds or worse_seeds == total_seeds:
            confidence = "high"
        ...
        classifications[lever] = {
            ...
            "per_seed": [
                round(r["final_eval"], 4)
                for r in variant_results
                if r.get("final_eval") is not None
            ],
...
def generate_report(results, classifications, outdir):
    ...
    for lever, info in sorted(classifications.items()):
        ...
        a(
            f"| {lever} | **{info['status']}** | {delta_str} | {info['confidence']} | {info['better_seeds']}/{info['worse_seeds']} |"
        )
    ...
    if working:
        ...
        for lever in working:
            ...
            a(
                f"- **{lever}**: delta = {info['delta_mean']:+.4f} (baseline {info['baseline_mean']:.4f} -> {info['variant_mean']:.4f})"
            )
    ...
    if working:
        a(
            "1. **Keep** the working mechanisms — they provide genuine quality improvement."
        )
    if neutral:
        a(
            f"2. **Remove** the {len(neutral)} neutral mechanisms — they add code complexity with zero benefit."
        )
    if harmful:
        a(
            f"3. **Remove** the {len(harmful)} harmful mechanisms — they actively degrade performance."
        )
    if not working and not harmful:
        a(
            "1. All mechanisms are neutral at this scale — consider testing at smaller model sizes (5M) where effects may be more pronounced."
        )
    a("")
...
def main():
    ...
    if args.results:
        ...
        for fname in [
            ...
            print(
                "No results found. Run with --run on Kaggle T4, or provide --results <path>"
            )
            sys.exit(1)
    ...
    print("\nGenerating report ...")
    generate_report(results, classifications, outdir)
    ...
    for lever, info in sorted(classifications.items()):
        if info["status"] == "baseline":
            ...
            print(
                f"  {lever:20s} {info['status']:10s}  delta = {info['delta_mean']:+.4f}  ({info['confidence']})"
            )
    print("=" * 72)

...
def run(dim, steps, seq, batch):
    ...
    torch.manual_seed(0)
    variants = [
        ("fix", "k=4  residual=x (current)", {"k": 4, "unique": 4, "depth": 12}),
        ("k1", "k=1  no memory needed", {"k": 1, "unique": 4, "depth": 12}),
        ("noshare", "k=4  12 distinct blocks", {"k": 4, "unique": 12, "depth": 12}),
    ]
    tasks = []
    ...
    recs, wall = _train_concurrent([t[3] for t in tasks], steps, [t[4] for t in tasks])
    results = {}
    for (sid, label, k, m, _tr, ev), rec in zip(tasks, recs):
        ...
        print(
            "[%-8s] %s train %.4f -> %.4f | span_ce %.4f span_acc %.4f "
            "pad_ce %.4f | pred|W-I| %.4f"
            % (
                sid,
                label,
                results[sid]["train_first"],
                results[sid]["train_last"],
                ce_span,
                acc_span,
                ce_pad,
    ...
    print(f"[concurrent] 3 diagnostic trainings in {wall:.1f}s")
...
def main():
    ...
    print("=" * 78)
    print(
        "recall root-cause diagnostic  |  dim=%d  steps=%d  seq=%d  batch=%d"
        % (dim, steps, seq, batch)
    )
    print(
        "V=%d  floor ln(V)=%.3f  (flat at floor == model learned NOTHING)" % (V, floor)
    )
    print("=" * 78)

    results, _wall, floor = run(dim, steps, seq, batch)

    print("\n" + "=" * 78)
    print(f"summary  (train last | span_ce | floor {floor:.3f})")
    for label, r in results.items():
        print(
            "  %-32s train %s -> %s   span_ce %s"

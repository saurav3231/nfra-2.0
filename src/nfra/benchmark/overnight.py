"""
OVERNIGHT — the final, very big and broad NFRA comparison.

Designed to run for a full Kaggle GPU session (T4/P100, ~9h). It compares
NFRA vs RWKV vs RetNet vs GPT-2 (Mamba-SSM optional, it's slow) on REAL
text only:
...
CFG = {
    "quick": {"steps": 300, "sizes": [5], "seeds": 1},
    "standard": {"steps": 600, "sizes": [5, 20], "seeds": 2},
    "big": {"steps": 1500, "sizes": [5, 10, 20, 50], "seeds": 3},
}
if MODE not in CFG:
...
PHASES = [
    ...
    for p in os.environ.get(
        "NFRA_OVN_PHASES", "core,context,efficiency,ablate,recall,deploy,perf,data2"
    ).split(",")
    if p.strip()
]
...
SIZES = [
    ...
    for x in os.environ.get(
        "NFRA_OVN_SIZES", ",".join(map(str, CFG[MODE]["sizes"]))
    ).split(",")
    if x.strip()
]
if not SIZES:
...
FAMILIES = [
    f.strip().lower()
    for f in os.environ.get("NFRA_OVN_FAMILIES", "nfra,rwkv,retnet,gpt2").split(",")
    if f.strip()
]
    ...
    for url in WIKI_URLS:
        try:
            ...
            if os.path.getsize(tmp) < 1024 * 1024:
                raise ValueError(
                    "download too small (%d bytes), likely an "
                    "error page" % os.path.getsize(tmp)
                )
            with zipfile.ZipFile(tmp) as z:
                for src, dst in [
                    ("wikitext-2-raw/wiki.train.raw", WIKI_FILES["train"]),
            ...
            last_err = e
            print(f"  [warn] mirror failed ({e!r}), trying next...")
            try:
                os.remove(tmp)
            except OSError:
                pass
        ...
        print(
            f"  [warn] zip mirrors failed ({last_err!r}) - trying parquet via "
            "huggingface_hub"
        )
        try:
            import pandas as pd
            ...
            raise SystemExit(
                f"[data] could not download WikiText-2 (zip: {last_err!r}; parquet: {pe!r}). "
                f"Place the two raw .txt files ({list(WIKI_FILES.values())}) in the working directory "
                "and re-run."
            )
...
def ensure_shakespeare():
    ...
    if not os.path.exists(dst):
        ...
        try:
            ...
            raise SystemExit(f"[data] could not download TinyShakespeare ({e!r})")
    return dst
...
from nfra.benchmark.global_arena import (
    _build_family as _build_fam_ga,
)
from nfra.benchmark.global_arena import (
    _run as _run_ga,
)
from nfra.utils.quantization import apply_int8_to_model
...
def plan_steps(label, n_runs, base_steps, min_steps=150):
    ...
    left = BUDGET.left()
    if left <= 180:
        print(f"  [budget] {left / 60:.0f} min left - skipping {label}")
        return 0
    ...
    k = (
        left
        * 0.6
        / max(n_runs * base_steps * max(EST_SEC_STEP.values(), default=ETA_DEF), 1e-6)
    )
    steps = int(base_steps * k)
    print(
        "  [budget] %s phase: %.0f min left, scaling %d -> %d steps"
...
def _primary_val_loader(max_batches=None):
    ...
    return DataLoader(
        _ds, batch_size=BATCH, shuffle=False, num_workers=0, pin_memory=HAS_CUDA
    )


def mb(model):
    return sum(v.numel() * v.element_size() for v in model.state_dict().values()) / 1e6
...
def assert_tokens_in_range(model, loader, vocab, label):
    ...
    if mx >= vocab:
        raise SystemExit(
            ...
            "WikiText-2 char files in the working directory." % (label, mx, vocab)
        )
...
def phase_core(vocab, random_loss):
        ...
        for seed in SEEDS:
            ...
            for fam in FAMILIES:
                ...
                assert_tokens_in_range(
                    m, train_loaders[seed], vocab, "core:%s@%dM" % (fam, size)
                )
                rec = train_one(
                    m,
                    vocab,
    ...
    for size in SIZES:
        ...
        for fam in FAMILIES:
            ...
            m_final, sd_final = mean_std(
                [r["eval_hist"][-1][1] for r in recs if r["eval_hist"]]
            )
            m_auc, _ = mean_std(
                [sample_auc(r["eval_hist"]) for r in recs if r["eval_hist"]]
            )
            ...
            row = {
                "params": params,
                "depth": specs[size][fam]["depth"],
                "spec": {k: v for k, v in specs[size][fam].items() if k != "builder"},
                "final_eval": m_final,
...
def phase_context(vocab):
    ...
    for fam in FAMILIES:
        ...
        print(
            "  [ctx] %-6s final %.3f | @256 %.3f  @512 %.3f  @1024 %.3f"
...
# ───────────────────────────── phase: efficiency ─────────────────────────────
def _train_energy(model, vocab, steps, tr, ev, eval_gap, budget, seed, ema_decay=0.0):
    model.train()
    ...
    return {
        "loss_hist": [float(v) for v in hist],
        "eval_hist": eval_hist,
...
def phase_efficiency(vocab):
    ...
    for b in ENERGY_BUDGETS:
        ...
        print(
            "  [energy] budget {:.2f} -> final {}  {:.0f} tok/s  {:.2f} GB".format(
                b, f"{final:.3f}" if final else "NA", rec["tok_s"], rec["peak_mem"]
            )
        )
    return rows
...
def phase_ablate(vocab):
    ...
    for name, build_kw, train_kw, n_seeds in ABLATE:
        ...
        if fam not in FAMILIES:
            print("  [skip] %-16s (family %s not in NFRA_OVN_FAMILIES)" % (name, fam))
            continue
...
def phase_recall():
    ...
    steps = plan_steps("recall", 2 * len(RECALL_KS), min(STEPS, 600), min_steps=120)
    if steps <= 0:
        return {}
    dim = int(os.environ.get("NFRA_OVN_RECALL_DIM", "224"))
    rows = recall_probe._run_all(
        RECALL_KS,
        steps=steps,
        dim=dim,
        seq_len=256,
...
def phase_deploy(vocab):
    ...
    for fam in FAMILIES:
        ...
        print(
            "  [int8] %-6s %.1f -> %.1f MB (%.0f%% saved) | eval %.3f -> %.3f "
            "| CPU %.0f tok/s"
            % (fam, mb_fp, mb_i8, rows[fam]["size_saved_pct"], fp_eval, i8_eval, cpus)
        )
    return rows
...
def phase_perf(vocab):
    ...
    for fam in FAMILIES:
        ...
        print(
            "  [perf] %-6s prefill %5.0f | gen %5.1f tok/s | %5.2f ms/tok "
            "| %.2f GB | 2x ctx %.3f"
            % (
                fam,
                pre,
...
def phase_data2(vocab=None):
    ...
    eval_loader = DataLoader(
        ev, batch_size=BATCH, shuffle=False, num_workers=0, pin_memory=HAS_CUDA
    )
    rows = {}
    for fam in FAMILIES:
        ...
        rows[fam] = {
            ...
            "random_loss": random_loss2,
            "tok_s": rec["tok_s"],
        }
        print(
            "  [data2] %-6s final %s (random %.2f)  %.0f tok/s"
            % (
                fam,
                (
                    "{:.3f}".format(rows[fam]["final_eval"])
                    if rows[fam]["final_eval"]
                    else "NA"
                ),
                random_loss2,
                rec["tok_s"],
...
def build_report(data, env):
    ...
    a(
        "**Data:** WikiText-2 (char, real text)%s  |  **Vocab:** %d  |  "
        "**Sizes:** %sM  |  **Seeds:** %s  |  **Steps:** %d  |  **Mode:** %s\n"
        % (
            " + TinyShakespeare" if "data2" in data else "",
            VOCAB,
            SIZES,
            SEEDS,
            STEPS,
    ...
    a(
        "**Families:** {} (param-matched)  |  "
        "**Optimizer:** AdamW 3e-4 (warmup+cosine)\n".format(" vs ".join(FAMILIES))
    )
    a("**Environment:** " + ", ".join(f"{k}={v}" for k, v in env.items() if v) + "\n")
    a(f"**Random-guess loss:** {RANDOM_LOSS:.3f} (ln vocab)\n\n")
    ...
    if "core" in data:
        ...
        rows = [
            [
                fam,
                fmt(scaling[fam]["slope"], 4),
                fmt(scaling[fam].get("r2"), 3),
                fmt(scaling[fam].get("loss_100m"), 3),
                fmt(scaling[fam].get("loss_1b"), 3),
                scaling[fam].get("n", 0),
            ]
            for fam in FAMILIES
        ]
        a(
            md_table(["family", "slope", "R²", "extrap @100M", "@1B", "points"], rows)
            + "\n"
        )
        for size in sorted(SIZES):
            m = mets[size]
            ...
            rows = [
                [
                    fam,
                    fmt(m[fam]["final_eval"], 3),
                    fmt(m[fam]["final_eval_sd"], 3),
                    fmt(m[fam]["ppl"], 2),
                    fmt(m[fam]["sample_auc"], 3),
                    f"{m[fam]['tok_s_train']:.0f}",
                    fmt(m[fam]["peak_mem"], 2),
                    fmt(m[fam].get("extrap_delta"), 3),
                ]
                for fam in FAMILIES
            ]
            a(
            ...
            for s in METRIC_SPEC:
                ...
                if w is None:
                    ...
                    v = m[w].get(s["key"])
                    rows.append(
                        [s["label"], w, fmt(v, 3) if isinstance(v, float) else v]
                    )
            a(
                "**Who wins which aspect @ %dM:**\n" % size
    ...
    if "context" in data:
        ...
        rows = [
            [
                fam,
                fmt(v["train_final"], 3),
                fmt(v["lengths"].get(256), 3),
                fmt(v["lengths"].get(512), 3),
        ...
        a(
            md_table(["family", "train", "@256", "@512", "@1024", "Δ@2x", "Δ@4x"], rows)
            + "\n"
        )
    ...
    if "efficiency" in data:
        ...
        a(
            md_table(["energy budget", "eval loss", "train tok/s", "peak GB"], rows)
            + "\n"
        )
    ...
    if "ablate" in data:
        ...
        a(
            md_table(
                ["variant", "eval", "Δ vs base", "tok/s", "Δ tok/s", "seeds"], rows
            )
            + "\n"
        )
    ...
    if "recall" in data:
        ...
        a(
            "*Diagnostic on synthetic structured associative recall (learnable); "
        ...
        for k in RECALL_KS:
            ...
            rows.append(row)
        hdr = ["k"] + [x for f in fams for x in (f + " span CE", f + " acc")]
        a(md_table(hdr, rows) + "\n")
    ...
    if "deploy" in data:
        ...
        rows = [
            [
                ...
                f"{v['mb_fp32']:.1f}",
                f"{v['mb_int8']:.1f}",
                f"{v['size_saved_pct']:.0f}%",
                fmt(v["eval_fp32"], 3),
                fmt(v["eval_int8"], 3),
                fmt(v["eval_delta"], 3, " Δ"),
                f"{v['cpu_prefill_tok_s_int8']:.0f}",
            ]
...
def main():
    if not HAS_CUDA:
        print(
            "[WARN] no CUDA — an overnight run is only meaningful on a Kaggle "
            "GPU. Results here are a smoke test only."
        )
    completed, data = load_state()

    print("=" * 72)
    print("  OVERNIGHT — final all-axes NFRA comparison")
    print("  NFRA vs {}  (param-matched, REAL text)".format(" vs ".join(FAMILIES)))
    print("=" * 72)
    print(
        "  mode     : %-8s steps: %d   sizes: %sM   seeds: %s"
        % (MODE, STEPS, SIZES, SEEDS)
    )
    print(
        "  data     : WikiText-2 (char)   vocab: %d   random loss: %.3f"
        % (VOCAB, RANDOM_LOSS)
    )
    print("  phases   : {}".format(",".join(PHASES)))
    print(
        "  device   : %s" % (torch.cuda.get_device_name(0) if HAS_CUDA else "CPU")
        + ("  (fp16 AMP)" if USE_AMP else "")
    )
    print(f"  budget   : {MAX_MIN:.0f} min   out: {OUTDIR}")
    print("=" * 72)
    if completed:
    ...
    for phase in PHASES:
        if phase in completed:
            print(f"\n[skip] {phase} already completed")
            continue
        if phase not in PHASE_FNS:
            print(f"\n[skip] unknown phase {phase!r}")
            continue
        if BUDGET.left() <= 60:
            print(
                f"\n[budget] {BUDGET.left() / 60:.0f} min left — stopping before {phase}"
            )
            break
        try:
            ...
            print(f"[phase {phase} done] report + results saved")
        except Exception as e:
            print(f"[PHASE {phase} FAILED] {e!r} (continuing)")
            traceback.print_exc()
    ...
    try:
        ...
        if "efficiency" in data:
            rows = [
                [v["budget"], v["final_eval"], v["tok_s"], v["peak_mem"]]
                for v in data["efficiency"].values()
            ]
            write_csv(
                "efficiency.csv",
        ...
        if "ablate" in data:
            rows = [
                [name, v["final_eval"], v["tok_s"], v["seeds"]]
                for name, v in data["ablate"].items()
            ]
            write_csv(
                "ablate.csv", ["variant", "eval_loss", "train_tok_s", "seeds"], rows
            )
        if "recall" in data:
        ...
        if "data2" in data:
            rows = [
                [fam, v["final_eval"], v["random_loss"], v["tok_s"]]
                for fam, v in data["data2"].items()
            ]
            write_csv(
                "data2.csv", ["family", "eval_loss", "random_loss", "tok_s"], rows
            )
        print(f"\n[out] CSVs written to {OUTDIR}")
    except Exception as e:
    ...
    print("\n" + "=" * 72)
    print(
        f"  OVERNIGHT GRAND ARENA DONE in {time.perf_counter() - t_all:.0f}s ({BUDGET.used_min():.0f} min)"
    )
    print(f"  -> {REPORT_FILE}")
    print(f"  -> {RESULTS_FILE}")
    print(f"  -> {STATE_FILE} (resume state)")
    print("=" * 72)

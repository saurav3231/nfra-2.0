...
GA_PHASES = [p.strip() for p in
             os.environ.get('NFRA_GA_PHASES', 'core,ablate,recall,perf').split(',')
             if p.strip()]
GA_SIZES = [int(x) for x in
            os.environ.get('NFRA_GA_SIZES', '5,20').split(',') if x.strip()]
...
def _run(model, vocab, steps, train_loader, eval_loader, eval_gap, **kw):
    ...
    try:
        return train_one(model, vocab, steps, train_loader, eval_loader,
                         eval_gap, **kw)
    except Exception as e:  # pragma: no cover
        print('  [ERROR] run failed: %r' % e)
        return {'loss_hist': [], 'eval_hist': [], 'tok_s': 0.0,
                'ms_per_step': 0.0, 'peak_mem': 0.0, 'nan_steps': 0,
                'wall_s': 0.0}
...
def _agg(recs):
    ...
    return {
        ...
        'sample_auc': m_auc, 'tok_s': m_tok, 'ms_per_step': m_ms,
        'peak_mem': m_mem,
...
def _build_family(fam, size, vocab, seed, **overrides):
    ...
    if fam == 'nfra':
        m = build_nfra(vocab, spec['dim'], spec['extra']['unique_blocks'],
...
def phase_core(data, vocab, random_loss):
    ...
    for size in GA_SIZES:
        ...
        for fam in FAMILIES:
            ...
            row['param_eff'] = ((random_loss - row['final_eval'])
                                / (row['params'] / 1e6)) if row['final_eval'] else None
...
def phase_perf(data, vocab):
    ...
    for fam in FAMILIES:
        ...
        out[fam] = {'prefill_tok_s': pre, 'gen_tok_s': gen['gen_tok_s'],
                    'ms_per_token': gen['ms_per_token'],
                    'infer_mem': gen['infer_mem']}
        print('  [perf] %-6s prefill %5.0f tok/s | gen %5.1f tok/s | '
              '%5.2f ms/tok | %.2f GB'
...
def build_report(all_data, vocab, random_loss):
        ...
        for size in GA_SIZES:
            for fam in FAMILIES:
                ...
                rows.append([f'{size}M', fam, fmt(m['final_eval'], 3),
                             ...
                             fmt(m['sample_auc'], 3), f"{m['tok_s']:.0f}",
                             fmt(m['peak_mem'], 2), fmt(m['nan_steps'], 0)])
        a(md_table(['size', 'family', 'eval', 'mean±std', 'ppl', 'AUC',
                    'train tok/s', 'peak GB', 'NaN steps'], rows))
        a('')
        a('\n### 1b. Scaling (bits of loss per doubling of params; neg = better)\n')
        rows = [[f, fmt(c['scaling'][f]['slope'], 4),
    ...
    if 'ablate' in all_data:
        ...
        for name, row in ab.items():
            if base and name != 'nfra_baseline':
                ...
                rows.append([name, fmt(row['final_eval'], 3),
                             fmt(dE, 3, ' Δ'), f"{row['tok_s']:.0f}",
                             fmt(dT, 1, '% Δ'), row['seeds']])
            else:
                rows.append([name, fmt(row['final_eval'], 3), '—',
        ...
        if base:
            ...
            if best[0] != 'nfra_baseline' and best[1]['final_eval'] and base['final_eval']:
                d = base['final_eval'] - best[1]['final_eval']
                a(f"**Best variant:** {best[0]} (eval {fmt(best[1]['final_eval'],3)}, "
                  f"{d:+.3f} vs baseline)\n")
    ...
    if 'recall' in all_data:
        ...
        for name, rr in r['results'].items():
            rows.append([rr.get('label', name), rr['train_first'], rr['train_last'],
                         rr['span_ce'], rr['span_acc'], rr['pred_identity_dev']])
        a(md_table(['variant', 'train first', 'train last', 'span CE', 'span acc',
                    'pred |W-I|'], rows))
        a('')
        ...
        if fix and fix['span_ce'] < r['floor'] - 0.03:
            a('**Recall verdict:** `fix` now LEARNS k=4 → the self-prediction '
              'residual was the root cause.\n')
        elif nosh and fix and nosh['span_ce'] < r['floor'] - 0.03 and fix['span_ce'] >= r['floor'] - 0.03:
            a('**Recall verdict:** `fix` still floors but `noshare` learns → '
              'depth weight-sharing is the culprit.\n')
        elif k1 and k1['span_ce'] < r['floor'] - 0.03:
            a('**Recall verdict:** k=1 learns but k=4 does not → memory '
              'formability; bet on memory levers (AFC-α).\n')
        else:
            a('**Recall verdict:** nothing learns → capacity/optimization at '
    ...
    if 'perf' in all_data:
        ...
        a(md_table(['family', 'prefill tok/s', 'gen tok/s (b=1)',
                    'ms/token', 'peak infer GB'], rows))
        a('')
...
def main():
    if not HAS_CUDA:
        print('[WARN] no CUDA — overnight run is meaningless on CPU. '
              'Run on Kaggle T4.')
    ...
    for phase in GA_PHASES:
        try:
            if phase == 'core':
                ...
                print('[skip] unknown phase %r' % phase)
                continue
            with open(OUT_JSON, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, default=float)
            print('[phase %s done] progress saved -> %s' % (phase, OUT_JSON))
        except Exception as e:
            print('[PHASE %s FAILED] %r (continuing to next phase)' % (phase, e))
            import traceback
            traceback.print_exc()
    ...
    print('\n' + '=' * 72)
    print('  GLOBAL ARENA DONE in %.0fs  -> %s  -> %s'
          % (time.perf_counter() - t_all, OUT_MD, OUT_JSON))
    print('=' * 72)

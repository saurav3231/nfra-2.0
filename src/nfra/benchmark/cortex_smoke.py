...
def check_exit_skip(model, vocab):
    ...
    return dict(max_abs_delta_depth1_vs_allexit=round(delta, 6),
...
def main():
    ...
    print('=' * 64)
    print('  NFRA 3.3 Cortex smoke test  (dim=%d unique=%d depth=%d vocab=%d)'
          % (dim, unique, NFRA_DEPTH, vocab))
    print('=' * 64)

    # ---- 1. build + params ----
    model = build_nfra(vocab, dim, unique, depth=NFRA_DEPTH,
                       use_cortex=True).to(DEVICE)
    rescale_embed(model)
    params = count_params(model)
    print('\n[1] build OK   params=%.4fM  (target 5M)' % (params / 1e6))
    block = model.layers[0]
    print('    block type : %s' % type(block).__name__)
    print('    has exit_gate : %s   has matrix mixer : %s'
    ...
    for s in range(1, steps + 1):
        ...
        if first is None:
            first = loss.item()
        last = loss.item()
    print('[2] train OK   loss %.4f -> %.4f  (random=%.3f)  nan_steps=%d'
    ...
    with torch.no_grad():
        ...
        print('[3] eval OK   logits %s finite=%s'
              % (tuple(logits.shape), _finite(logits)))

    # ---- 4. exit-gate skip ----
    result = check_exit_skip(model, vocab)
    print('[4] exit skip : %s' % result)
